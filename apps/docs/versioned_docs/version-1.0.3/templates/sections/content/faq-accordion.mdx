...
The FAQ Accordion is designed to be both "batteries-included" for quick setup and fully composable for custom requirements.

### Quick Start
Use the `FAQSection` component for a complete, feature-rich FAQ section with minimal code.

```tsx
import { FAQSection } from '@ignix-ui/faq-accordion';

const items = [
  { 
    id: '1', 
    question: 'How do I install Ignix UI?', 
    answer: 'You can install it via our CLI: `npx ignix init`.' 
  },
  { 
    id: '2', 
    question: 'Is there a dark mode?', 
    answer: 'Yes, all components support multiple theme variants including dark and midnight.' 
  }
];
...
### Custom Composition
For full control over the layout and behavior, use the compound components.
...
import { 
  ...
  AccordionTitle, 
  AccordionDetails 
} from '@ignix-ui/faq-accordion';

export default function CustomFAQ() {
  return (
    <Accordion variant="card" animationVariant="slide">
      <AccordionItem id="item-1">
        <AccordionSummary id="item-1">
          <AccordionTitle>Custom Header Logic</AccordionTitle>
        </AccordionSummary>
        <AccordionDetails id="item-1">
          The details content can include any React components, 
          allowing for complex FAQ answers.
        </AccordionDetails>
      </AccordionItem>
    </Accordion>
  );
