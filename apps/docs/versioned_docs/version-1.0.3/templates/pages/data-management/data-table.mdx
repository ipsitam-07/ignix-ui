...
export function DataTable<T extends Record<string, any>>({
  ...
  const getMinTableHeight = useCallback(() => {
    const rowHeight = 52; // py-3 (12px*2) + content (~28px)
...
| `variant` | `'default' \| 'bordered' \| 'minimal' \| 'elevated'` | `'default'` | Visual style of the table container. |
...
| `mobileBreakpoint` | `number` | `768` | Pixel width below which the mobile card layout is rendered instead of the table. |
...
// toggleSort(key) cycles: none → asc → desc → none
```
