...
The `server` variant provides a specialized theme for 500 errors, featuring a dark background with a grid pattern and red glow effects.

```tsx
function ServerErrorPage() {
  const errorReferenceId = "ERR-X92J-L10";

  return (
    <ErrorPage variant="server" icon={Server}>
      <ErrorPageContent>
        <ErrorPageIcons icons={[AlertTriangle, Wrench, Activity, ShieldAlert]}>
          <ErrorPageErrorCode errorCode="500" animationType="glow" />
        </ErrorPageIcons>
        <ErrorPageHeading>Internal Server Error</ErrorPageHeading>
        <ErrorPageDesc>
          Something went wrong on our end. Our technical team is investigating.
        </ErrorPageDesc>
        <ErrorPageErrorReference errorReferenceId={errorReferenceId} />
        <ErrorPageLinks>
          <ButtonWithIcon variant="primary" icon={<RefreshCw />}>
            Retry
          </ButtonWithIcon>
...
|------|------|---------|-------------|
| `variant` | `"default" \| "minimal" \| "gradient" \| "dark" \| "server"` | `"default"` | Visual variant of the error page |
