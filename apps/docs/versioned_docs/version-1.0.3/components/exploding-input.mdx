...
The `ExplodingInput` component is a delightful, interactive input field that renders performant canvas-based particle explosions as the user types. It features typing-speed detection, cursor tracking, physics-based animations, and optional audio effects, all optimized using `requestAnimationFrame`, reduced-motion media queries, and off-screen texture rendering.
...
The `ExplodingInput` accepts all standard standard HTML `<input>` attributes, plus the following specialized configuration options:

| Prop                 | Type                                                                     | Default     | Description                                                                 |
|----------------------|--------------------------------------------------------------------------|-------------|-----------------------------------------------------------------------------|
| `triggerMode`        | `"keypress" \| "submit" \| "focus" \| "clear" \| "custom"`               | `"keypress"`| Determines what action triggers the explosion.                              |
| `particlePreset`     | `"confetti" \| "sparks" \| "stars" \| "bubbles" \| "letters" \| "emoji"` | `"confetti"`| Specifies the visual style of the spawned particles.                        |
| `customEmoji`        | `string[]`                                                               | `DEFAULT_EMOJI` | An array of emoji characters to use when `particlePreset="emoji"`.      |
| `characterParticles` | `boolean`                                                                | `false`     | If true, the typed character itself becomes the explosion particle.         |
| `direction`          | `"up" \| "down" \| "left" \| "right" \| "radial" \| "burst"`             | `"up"`      | Sets the direction velocity of the particle spread.                         |
| `cursorTrail`        | `boolean`                                                                | `false`     | Whether to spawn a trailing spark effect continuously while focused.        |
| `validate`           | `(value: string) => boolean`                                             | `undefined` | A function that changes particle color to green/red based on validity.      |
| `maxParticles`       | `number`                                                                 | `400`       | The maximum number of canvas particles alive at once before pooling starts. |
| `audio`              | `"pop" \| "whoosh" \| "sparkle" \| string`                               | `undefined` | Triggers a sound effect hook. Can be a builtin preset or a URL to an `.mp3`/`.wav`. |
