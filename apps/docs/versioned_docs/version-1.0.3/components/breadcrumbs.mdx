...
| `separatorIcon` | `React.ElementType` | `ChevronRight` | Custom icon to use as a separator between breadcrumb items |
| `steps` | `string[]` | `[]` | Array of step labels (used with 'step' and 'progress' variants) |
| `currentStep` | `number` | `0` | Current active step index (0-indexed) |
...
| `asChild` | `boolean` | `false` | If true, the component will merge its props onto its immediate child |